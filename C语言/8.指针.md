记住：`int*** p`，不管 `p` 前面有几个 `*` ，`p` 永远是地址，`*p` 也永远可以取到值（前提是指针被初始化过）
...
4. 在函数参数中使用指针访问数组
   ...
   -  1：传递指向**首元素的指针**
     ...
     ```c
     void printArray(int* arr, int n){      // 接收一级指针
         for(int i=0;i<n;i++)
             printf("%d ", arr[i]);
     }
   ...
   -  2：传递指向整个数组的指针
     ...
     ```c
     void printArray(int(* arr)[5]){        // 接收二级指针
         for(int i=0;i<5;i++)
             printf("%d ", (*arr)[i]);
     }
...
2. 一级指针参数 + 二维数组
   ...
   ```c
   void func(int* p, int rows, int cols) {            // 接收一级指针
...
2. 动态二维数组
   ...
   int row1[3] = {1,2,3};
   int row2[3] = {4,5,6};
   int* arr[2] = {row1, row2};   // 指针数组，arr是二维数组
   int** p = arr;                // 二级指针
