...
2. 准备实体类 (`Getter`、`Setter`、`toString`方法自己写)
...
3. 创建 `Mapper` 接口和 `Mapper` 的 `XML` 文件
...
1. 单个简单类型
   ...
   - `#{}`：`Mybatis` 会将 `SQL` 语句中的 `#{}` 转换为问号**占位符**
   - `${}`：`${}` 底层 `Mybatis` 做的是**字符串拼接**操作

   通常不会采用 `${}` 的方式传值。一个特定的适用场景是：通过 `Java` 程序动态生成数据库表，表名部分需要 `Java` 程序通过参数传入；而 `JDBC` 对于**表名部分是不能使用问号占位符的**，此时只能使用 `${}`
...
2. 多个简单类型
   ...
   1. 方案1：`MyBatis` 默认机制，使用 **`#{param1}` `#{param2}`**的方式定义变量或者**`#{arg0}` `#{arg1}`**
...
当参数是实体类型时(实体对象或者 `Map` )，传入实体的**属性名**
...
此时不需要任何特殊处理，在 `resultType` 属性中还是设置**实体类类型**即可
...
2. 非自增长类型主键
   ...
   则可以使用 `selectKey` 子元素：**`selectKey` 元素将会首先运行**，`id` 会被设置，然后插入语句会被调用
...
`resultMap`：`id`：`resultMap`标识，`type`：返回值类型
