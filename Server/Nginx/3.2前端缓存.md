...
1. **禁用前端强缓存 `index.html`**
   ...
   ```nginx
   # 只针对 index.html 文件禁用强缓存
   location = /index.html {
       add_header Cache-Control "no-cache, no-store, must-revalidate";   # 如果不考虑老浏览器兼容，只配置 Cache-Control 就可以
       add_header Pragma "no-cache";          # HTTP/1.0 遗留，针对老代理
       add_header Expires "0";                # HTTP/1.0 遗留，针对老浏览器
   }
   ```
   ...
   # 有 alias 的情况
   location = /index.html {
       alias /opt/projectname/index.html;
       add_header Cache-Control "no-cache, no-store, must-revalidate";   # 不需要兼容的话，可以只配置这一个
   }
   ```
...
2. **配置强缓存时间**
   ...
   ```nginx
   location /assets/ {
       add_header Cache-Control "public, max-age=31536000, immutable";     # 配置缓存 1 年
   }
   ```

3. 禁用协商缓存，默认开启状态；无特殊需求不要禁用
...
**需要在 `nginx` 中配置，每个浏览器没有固定缓存时长策略，不便推测**
