...
4. **对大型复杂应用更友好**
   ...
   状态变化时，**React 组件**整体重渲染，可控性更强，避免了Vue局部更新的隐士依赖问题（更新机制不同，Vue更精细，但是也会有隐士依赖问题）
...
1. **组件化**：构建可复用的 UI 组件，并将其组合成复杂的用户界面
...
2. 解决`Class`组件生命周期导致逻辑分散问题

3. 更符合 React 当前和未来的发展方向

4. 开发者封装**自定义 Hooks** 的目的，
...
1. 明确「组件的定位 & 边界」，梳理真实使用场景

   需要一个多例，高可用的 `Message`

2. 设计 API（克制 & 稳定），方便程序员尽量简单的调用，也可以兼容复杂情况下的自定义使用

   简单+多态：可以使用 `message.info('hello')`、也可以使用 `message.info({ content: 'hello' })` 调用

3. 状态模型设计与性能优化

   1. `message.open`给消息队列增加消息
   2. 消息队列根据消息的`id`进行`diff`，哪些需要渲染，哪些需要删除
   3. 渲染消息
   4. 在此基础上去做`message.warning`、`Hooks`兼容封装、`.then` 调用封装
   5. 性能优化：单例容器，多例消息，减少容器的创建次数
