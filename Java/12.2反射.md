...
`Java` 中许多对象在运行时会出现两种类型：如编译时的类型、运行时的类型
...
| `public <A extends Annotation> A getAnnotation(Class<A> annotationClass)` | 此方法返回该元素的注解在此元素的指定注释类型（如果存在），否则返回 `null` |
...
| 方法                                                         | 说明                                                    |
| ------------------------------------------------------------ | ------------------------------------------------------- |
| `public Method getMethod(String name, Class <?>... parameterTypes)` | 返回 `Class` 对象所对应的指定参数列表的 `public`方法    |
